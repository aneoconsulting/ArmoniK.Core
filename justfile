...
} else if worker == "segfault" {
  "dockerhubaneo/armonik_core_segfault_test_worker" + ":" + tag
} else {
...
image_client_segfault   := env_var_or_default('SEGFAULT_CLIENT_IMAGE', "dockerhubaneo/armonik_core_segfault_test_client")
...
export SEGFAULT_CLIENT_IMAGE    := image_client_segfault + ":" + tag
...
# Build Segfault Client
buildSegfaultClient: (build SEGFAULT_CLIENT_IMAGE  "./Tests/Segfault/Client/src/Dockerfile")
