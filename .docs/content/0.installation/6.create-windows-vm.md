...
```
just -v tag=$env:VERSION object=local worker=htcmock ingress=false prometheus=false grafana=false seq=false queue=rabbitmq091 deploy
```

- The **env:VERSION** corresponds to the version of the Armonik.Core image you want to deploy. This should match the version of the image that has been built and pushed to your container registry. For example, it could be something like `0.31.0`.
