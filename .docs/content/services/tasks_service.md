...
sequenceDiagram
  ...
  cp ->>+ r: Get completed task input results
  r -->>- cp: #

  cp ->>+ t: Get tasks where input results are completed
  t -->>- cp: #
...
sequenceDiagram
  ...
  cp ->>+ a: Activate cancellation of tasks running on agents
  a -->>- cp: #
...
2. **Change Task Status**: The Control Plane updates the status of the tasks in the ITaskTable to "Cancelling".
