...
public class ResultTable : IResultTable
{
                              ...
                              CancellationToken cancellationToken = default)
  {
    ...
    var res = await resultCollection
                    .UpdateOneAsync(Builders<Result>.Filter.Where(model => model.Id == Result.GenerateId(sessionId, key) && model.OwnerTaskId == ownerTaskId),
                              ...
                              CancellationToken cancellationToken = default)
  {
    ...
    var res = await resultCollection.UpdateOneAsync(Builders<Result>.Filter.Where(model => model.Id == Result.GenerateId(sessionId, key) && model.OwnerTaskId == ownerTaskId),
                                                    Builders<Result>.Update.Set(model => model.Status,
                                                                                ResultStatus.Completed),
                                                    cancellationToken: cancellationToken)
                                    .ConfigureAwait(false);
                                 ...
                                 CancellationToken cancellationToken = default)
  {
    ...
    var result = await resultCollection.DeleteOneAsync(model => model.Id == Result.GenerateId(session, key),
                                                       cancellationToken)
                                       .ConfigureAwait(false);
