                                           ...
                                           Options.MongoDB            options)
  {
    ...
    if (options.UseMinimalIndexes == false)
    {
      indexModels.AddRange([
        IndexHelper.CreateHashedIndex<TaskData>(model => model.OwnerPodId), IndexHelper.CreateAscendingIndex<TaskData>(model => model.CreationDate,
                                                                                                                       expireAfter:
                                                                                                                       options.DataRetention),
        IndexHelper.CreateAscendingIndex<TaskData>(model => model.SubmittedDate),
        IndexHelper.CreateAscendingIndex<TaskData>(model => model.StartDate), IndexHelper.CreateAscendingIndex<TaskData>(model => model.EndDate),
