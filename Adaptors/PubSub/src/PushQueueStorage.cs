...
internal class PushQueueStorage : IPushQueueStorage
{
                                      ...
                                      CancellationToken        cancellationToken = new())
  {
    ...
    var tasks = messages
    .GroupBy(m => m.Options.Priority)
    .SelectMany(group =>
        group.Chunk(500)
             .Select(chunk =>
             {
                 var topic = $"a{options_.Prefix}-{partitionId}-{group.Key}";
                 var topicName = TopicName.FromProjectTopic(options_.ProjectId, topic);
                 return (topicName, chunk);
             }))
    .ParallelForEach(
  new ParallelTaskOptions(options_.DegreeOfParallelism),
  async item =>
  {
    await Publish(item.topicName, item.chunk);
  });
  }
