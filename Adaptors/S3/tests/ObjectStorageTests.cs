...
public class ObjectStorageTests : ObjectStorageTestBase
{
  ...
  private static readonly string SolutionRoot =
    Path.GetFullPath(Path.Combine(Path.GetDirectoryName(Path.GetDirectoryName(Path.GetDirectoryName(typeof(ObjectStorageTests).Assembly.Location))) ?? string.Empty));

  private static readonly string S3Path =
    $"{Path.DirectorySeparatorChar}Adaptors{Path.DirectorySeparatorChar}S3{Path.DirectorySeparatorChar}src{Path.DirectorySeparatorChar}bin{Path.DirectorySeparatorChar}Debug{Path.DirectorySeparatorChar}net8.0{Path.DirectorySeparatorChar}ArmoniK.Core.Adapters.S3" +
    ".dll";
  ...
  protected override void GetObjectStorageInstance()
  {
    Dictionary<string, string?> minimalConfig = new()
                                                {
                                                  {
                                                    "Components:ObjectStorageAdaptorSettings:ClassName", "ArmoniK.Adapters.S3.ObjectBuilder"
                                                  },
                                                  {
