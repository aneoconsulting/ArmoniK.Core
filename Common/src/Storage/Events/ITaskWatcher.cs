...
public interface ITaskWatcher : IInitializable
{
  ...
  /// </summary>
  /// <param name="filter">Filter expression describing the tasks events to be returned</param>
  /// <param name="cancellationToken">Token used to cancel the execution of the method</param>
  /// <returns>
  ///   A <see cref="IAsyncEnumerable{NewTask}" /> that holds the updates when they are available
  /// </returns>
  Task<IAsyncEnumerable<NewTask>> GetNewTasks(Expression<Func<TaskData, bool>> filter,
  ...
  /// </returns>
  Task<IAsyncEnumerable<TaskStatusUpdate>> GetTaskStatusUpdates(Expression<Func<TaskData, bool>> filter,
                                                                CancellationToken                cancellationToken = default);
}
