...
namespace ArmoniK.Core.Common.Tests.Auth
{
   ...
   * of AuthenticationSource to the corresponding interface implementation */
    ...
    [TestCase("CNUser1", "Fingerprint1", 0)]
    [TestCase("CNUser2",
              "Fingerprint2",
              1)]
    [TestCase("CNUser3",
    ...
    [TestCase("CNUser2",
              "Fingerprint7",
              1)]
    public void GetIdentityFromCnAndFingerprintShouldSucceed(string cn, string fingerprint, int userid)
    {
      ...
      Assert.AreEqual(Users[userid].UserId, ident.UserId);
    }
    ...
    [TestCase(0, "User1"),
    TestCase(1, "User2")]
    public void GetIdentityFromIdShouldSucceed(int id, string username)
    {
      var ident = AuthenticationSource.GetIdentityFromIdAsync(Users[id].UserId,
                                                              CancellationToken.None)
                                      .Result;
      Assert.NotNull(ident);
    ...
    [TestCase("User1", 0), TestCase("User2", 1)]
    public void GetIdentityFromNameShouldSucceed(string name, int id)
    {
    ...
    [TestCase("User1", "Role1", true),
     TestCase("User1", "Role2", false),
    TestCase("User2", "Role1", true),
    TestCase("User2", "RoleDontExist", false)]
    public void UserHasRoleShouldMatch(string username,
                            string rolename, bool hasRole)
    {
      var identity = AuthenticationSource.GetIdentityFromNameAsync(username, CancellationToken.None).Result;
      Assert.NotNull(identity);
      Assert.AreEqual(identity.IsInRole(rolename), hasRole);
    }

    [TestCase(                  "User1", "category1:name1", true),
    TestCase(                   "User1",
             "category1:name2", true),
    TestCase(                   "User1",
             "category1:name3", false),
    TestCase(                   "User2",
             "category1:name2", true), 
    TestCase("User2",
             "category1:name2:"+Permissions.AdminScope,
                                        ...
                                        string claim, bool hasClaim)
    {
      ...
      Assert.NotNull(identity);
      Assert.AreEqual(identity.HasClaim(c => c.Type == expected.Type && (expected.Value != null || c.Value == expected.Value)), hasClaim);
    }
