...
internal static class Program
{
  private static async Task Main()
  {
    ...
    if (!benchOptions.WaitResult)
    {
      logger.LogInformation("Tasks submitted in {Submitted}",
    ...
    if (benchOptions.DownloadResult)
    {
    ...
    if (benchOptions.PurgeData)
    {
      await channelPool.WithInstanceAsync(channel => new Sessions.SessionsClient(channel).PurgeSessionAsync(new PurgeSessionRequest
                                                                                                            {
                                                                                                              SessionId = createSessionReply.SessionId,
                                                                                                            }),
                                          CancellationToken.None)
