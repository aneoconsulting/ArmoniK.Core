...
# the median over the rounds, its spread and its gap to foldhash, the default.
#
...
for v in "${variants[@]}"; do
  features=()
  [[ "$v" == fold ]] || features=(--features "hash-$v")
  echo "build $v"
...
awk -F '\t' -v order="${variants[*]}" '
  ...
  END {
    flush()
    nv = split(order, vs, " ")
    printf "%-52s", "cycles/s: median ±half spread, gap to fold"
