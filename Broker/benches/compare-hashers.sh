...
# the median over the rounds, its spread and its gap to FxHash, the default.
#
...
awk -F '\t' -v order="${variants[*]}" '
  ...
  END {
    flush()
    nv = split(order, vs, " ")
    printf "%-52s", "cycles/s: median ±half spread, gap to fx"
